...
void write_music_on_file(struct Elenco *e, const char *filename) {
    ...
    fprintf(file, "Elenco di brani:\n");
    for (int i = 0; i < e->length; ++i) {
        struct Brano* brano = e->Elenco[i];
        fprintf(file,"Brano %d:\n", i + 1);
        fprintf(file,"Titolo: %s\n", brano->titolo);
        fprintf(file,"Autore: %s\n", brano->autore);
        fprintf(file,"Durata in secondi: %d\n", brano->durata_in_sec);
        fprintf(file,"Riproduzioni: %d\n", brano->rips);
        fprintf(file,"\n");
    }
}
